...
var KID_MARGIN = 10;
  ...
  /**
   * Controls to create a new puzzle & possibly save it.
   * Yo.
   *
   * @constructor
   */
  pzv.PuzzleMakerView = function($eventRelay) {
    var that = this;
    this.view = $('<div class=puzzleMaker></div>');

    var button = new common.ui.Button("Make a new puzzle");

    this.addChild(button);
    button.click(function() {
      $eventRelay.triggerHandler('makeNewPuzzle', [4, 3]);
    });
  };
  common.util.inherits(pzv.PuzzleMakerView, common.ui.View);
  ...
  pzv.MainView.prototype.__setupWidgets = function() {
    var that = this;
    var puzzleMakerView = new pzv.PuzzleMakerView($(this));
    this.addChild(puzzleMakerView);

    var dropdownItems = [];

    _.each(pzm.gameInstance.puzzleStructs, function(ps) {
      var item = {};
      item.text = ps.name;
      item.data = ps;
      item.selected = (pzm.gameInstance.currentPuzzleStruct ==  ps);
      selected = false;
      dropdownItems.push(item);
    });
    var dropdownOptions = {
      items: dropdownItems,
      classes: 'puzzleDropdown',
      name: 'dropdown',
      label: "select a puzzle",
    }

    this.dropdown = new common.ui.Dropdown(dropdownOptions);
    this.addChild(this.dropdown);
    this.dropdown.change(function() {
      that.__loadSelectedPuzzle();
    });
  ...
  pzv.MainView.prototype.__loadSelectedPuzzle = function() {
    var item = this.dropdown.currentItem();
    pzm.gameInstance.currentPuzzleStruct = item.data;
  ...
  pzv.PuzzleView = function($eventRelay) {
    ...
    $(pzm.gameInstance).bind('newPuzzleLoaded', function() {
      that.__newPuzzle();
    });
  };
  ...
  pzv.PuzzleView.prototype.resetKids = function() {
    ...
    var allObjects = this.puzzle.grid.getAllObjects();
  ...
  pzv.PuzzleView.prototype.addKid = function(kidGroup) {
    var kidView = new pzv.KidView(kidGroup, this, this.$eventRelay);
    this.addChild(kidView, this.$grid);
    kidView.render();
  };

  pzv.KidView = function(kidGroup, parentPuzzleView, $eventRelay) {
    var that = this;
    ...
    $(kidGroup).bind('countChanged', function() {
      that.render();
    });
    ...
    this.view.bind(window.FLAGS.click, function() {
      that.$eventRelay.triggerHandler('setKidType',
        [that.kidGroup.kidType]);
    });
